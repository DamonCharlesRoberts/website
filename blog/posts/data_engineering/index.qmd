...
```{mermaid}
%%| label: fig-csv-process
%%| fig-cap: ETL with CSV files
%%| fig-width: 6.5
%%{init: {'theme':'base', 'themeVariables':{'primaryColor':'#ffffff', 'primaryBorderColor': '#000000'}}}%%
flowchart TD;
A[Download raw data] --> B[Split into multiple CSV files with unique identifier columns]
B --> C[Clean data from the CSV files using unique identifier column while keeping it in separate files]
C --> D[Analyze data with cleaned data]
```

```{mermaid}
%%| label: fig-db-process
%%| fig-cap: ETL with DuckDB files
%%| fig-width: 6.5
%%{init: {'theme':'base', 'themeVariables':{'primaryColor':'#ffffff', 'primaryBorderColor': '#000000'}}}%%
flowchart TD;
A[Download raw data] -->B["Convert into a .db file: <br/> library(DBI) <br/> connection <- dbConnect(DuckDB::DuckDB(), 'db_file.DuckDB') <br/> dbExecute(connection, ''CREATE TABLE rawData AS SELECT * FROM 'raw_data.csv''')"]
B --> C["Load from raw data table <br/> raw_df <- dbGetQuery(connection, ''SELECT * FROM rawData'')"]
C --> D[Clean data]
D --> E["Store each table: <br/> dbWriteTable(connection, ''secureData'', secure_data_df) <br/> dbWriteTable(connection, ''demographicData'', demographic_data_df) <br/> dbWriteTable(connection, ''politicalData'', political_attitudes_df)"]
E --> F["Analyze data: <br/> demographic_df <- dbGetQuery(connection, ''SELECT * FROM demographicData'') <br/> political_attitudes_df <- dbGetQuery(connection, ''SELECT * FROM politicalData'') <br/> df <- merge(demographic_df, political_attitudes_df, by = ''unique_id'')"]
```

@fig-csv-process and @fig-db-process provides a visual representation of what the integration of the ETL process looks like for academics. Doing these steps summarized in the figures adds only one or two extra steps and necessitates learning one or two commands in SQL, but makes dramatic improvements on the security and reproducibility of one's data. With the simplicity of these extra steps but the dramatic improvements they provide, I think quantitative researchers could dramatically improve their data management practices!
